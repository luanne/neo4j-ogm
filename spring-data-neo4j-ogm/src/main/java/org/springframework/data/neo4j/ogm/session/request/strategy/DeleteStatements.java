package org.springframework.data.neo4j.ogm.session.request.strategy;

import org.springframework.data.neo4j.ogm.cypher.statement.ParameterisedStatement;
import org.springframework.data.neo4j.ogm.session.Utils;
...
public class DeleteStatements {
    ...
    public ParameterisedStatement delete(Long id) {
        return new ParameterisedStatement("MATCH (n) WHERE id(n) = { id } OPTIONAL MATCH (n)-[r]-() DELETE r, n", Utils
            .map("id", id));
    }
